...
field_dir = [[0,0,(-dipole_gap/2).value], [0,0,(dipole_gap/2).value]]
lump_port = Set_lumped_port("1",rect_port, field_dir)

model.add_action(lump_port)

##create radiation box
rad_rule = wavel.to_value(apu.m)/4     ##rule of thumbs.. 

##
##For some reasone even if you are using mm as units, if you add a dimensionless 
##quanity it automatically uses mts..
rad_width = 2*rad_rule+2*wire_radius    
rad_height =2*rad_rule+dipole_height
